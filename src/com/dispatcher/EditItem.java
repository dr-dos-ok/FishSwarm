...
public class EditItem extends JDialog implements ActionListener {
	...
	public EditItem(JFrame parent, String path) {
		...
		domainList.setSelectedItem(item.getDomain());
		itemText.setText(item.getItems());
	...
	void preparedata() {
		...
		try {
			...
			item = new Item(file.getName().substring(0,
					file.getName().lastIndexOf('.')), contents.toString().split(System.getProperty("line.separator"))[0], contents.toString().split(System.getProperty("line.separator"))[1]);
